    ...
    if form.validate_on_submit():
        ...
        if not os.path.exists(path):
            ...
            if "HEADR" in head[0] and "TRAIL" in cons.iloc[-1, 0]:
                ...
                head = head.tolist()
                new_headr = Headr(file_type=head[1], company_id=head[2], fileCreation_date_time=datetime.strptime((head[3].split(sep, 1)[0] + head[4].split(sep, 1)[0]), '%Y%m%d%H%M%S'),
                                  file_number=head[5])
                ...
                for j, i in cons.iterrows():
                    ...
                    new_consu = Consu(meter_number = meter_number, measurement_date_time = measurement_date_time, consumption=consumption, file_number=file_number)
